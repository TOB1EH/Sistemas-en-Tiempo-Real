...
void manejador_interrupcion_boton(int gpio, int level, uint32_t tick)
{
    if (level == 0) // Flanco de bajada (Botón presionado)
    {
        ...
        {
            // Diferencia entre cuando se prende el led y cuando realmente se detecto la pulsacion del boton.
            latencias[pulsaciones] = tiempo_accion - tick; // Calcular latencia y guardarla en el arreglo de latencias. Esto se ob
            pulsaciones++;
